...
export function calculatePercantageChange(current: number, previous: number) {
  if (previous === 0) {
    return previous === current ? 0 : 100; // Avoid division by zero
  }
...
export function fillMissingDays(
  ...
  const allDayes = eachDayOfInterval({ start: startDate, end: endDate });

  const transactionDays = allDayes.map((day) => {
